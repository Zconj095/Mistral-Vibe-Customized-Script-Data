...
from vibe.core.codebase import CodebaseManager
...
class CodebaseContextMiddleware:
    def __init__(self, manager: CodebaseManager) -> None:
        self.manager = manager
        self._last_user_index: int | None = None

    async def before_turn(self, context: ConversationContext) -> MiddlewareResult:
        if not context.config.codebase.enabled:
            return MiddlewareResult()

        user_index, user_content = _last_user_message(context.messages)
        if user_index is None:
            return MiddlewareResult()
        if self._last_user_index == user_index:
            return MiddlewareResult()

        self._last_user_index = user_index
        if not user_content:
            return MiddlewareResult()
        if "<CODEBASE_CONTEXT>" in user_content or "<CODEBASE_INDEX>" in user_content:
            return MiddlewareResult()

        context_block = await asyncio.to_thread(self.manager.build_context, user_content)
        if not context_block:
            return MiddlewareResult()
        return MiddlewareResult(
            action=MiddlewareAction.INJECT_MESSAGE, message=context_block
        )
