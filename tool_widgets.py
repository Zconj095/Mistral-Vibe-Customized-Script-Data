...
class ArtifactApprovalWidget(ToolApprovalWidget):
    def compose(self) -> ComposeResult:
        name = self.data.get("name", "")
        extension = self.data.get("file_extension", "text")
        content = self.data.get("content", "")
        ...
        yield Static("")
        yield Markdown(f"```{extension}\n{content}\n```")


class ArtifactResultWidget(ToolResultWidget):
    def compose(self) -> ComposeResult:
        MAX_LINES = 12
        message = self.data.get("message", "")
