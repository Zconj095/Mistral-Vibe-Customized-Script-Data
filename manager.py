...
if TYPE_CHECKING:
    ...
    from vibe.core.tools.mcp import RemoteTool
...
class ToolManager:
    ...
    async def _register_http_server(self, srv: MCPHttp | MCPStreamableHttp) -> int:
        ...
        try:
            ...
            logger.warning("MCP HTTP support unavailable: %s", exc)
            return 0
    ...
    async def _register_stdio_server(self, srv: MCPStdio) -> int:
        ...
        try:
            ...
            logger.warning("MCP stdio support unavailable: %s", exc)
            return 0
    ...
    def get_tool_config(self, tool_name: str) -> BaseToolConfig:
        ...
        if self._config.project_mode.enabled:
            if tool_name in {"read_file", "read_file_full", "read_pdf_code"}:
                merged_dict["max_read_bytes"] = self._config.project_mode.max_read_bytes
            elif tool_name == "write_file":
                merged_dict["max_write_bytes"] = (
                    self._config.project_mode.max_write_bytes
                )
            elif tool_name == "search_replace":
                merged_dict["max_content_size"] = (
                    self._config.project_mode.max_edit_bytes
                )
