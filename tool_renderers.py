...
from vibe.cli.textual_ui.widgets.tool_widgets import (
    ...
    ArtifactApprovalWidget,
    ArtifactResultWidget,
    TodoApprovalWidget,
    TodoResultWidget,
    ToolApprovalWidget,
    ToolResultWidget,
...
class ArtifactRenderer(ToolRenderer):
    def get_approval_widget(
        self, tool_args: dict
    ) -> tuple[type[ArtifactApprovalWidget], dict[str, Any]]:
        data = {
            "name": tool_args.get("name", ""),
            "content": tool_args.get("content", ""),
            "file_extension": tool_args.get("extension", "text"),
        }
        return ArtifactApprovalWidget, data

    def get_result_widget(
        self, display: ToolResultDisplay, collapsed: bool
    ) -> tuple[type[ArtifactResultWidget], dict[str, Any]]:
        data = {
            "success": display.success,
            "message": display.message,
            "path": display.details.get("path", ""),
        ...
        return ArtifactResultWidget, data
...
_RENDERER_REGISTRY: dict[str, type[ToolRenderer]] = {
    ...
    "bash": BashRenderer,
    "grep": GrepRenderer,
    "create_artifact": ArtifactRenderer,
}
